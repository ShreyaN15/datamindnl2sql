#!/usr/bin/env python3
"""
Test: List all the courses in which john is enrolled in
Bug: SELECT course_code FROM courses WHERE course_name = 'john'
Expected: Join students -> enrollments -> courses and filter by student first_name
"""
...
def test_courses_by_john():
    corrector = SQLPatternCorrector()
    
    question = "list all the courses in which john is enrolled in"
    
    # Bug: Looking for course_name = 'john' instead of student name
    generated_sql = "SELECT course_code FROM courses WHERE course_name = 'john'"
    ...
    bad_pattern = "course_name = 'john'"
    print(f"  3. No 'course_name = john': {'✅' if bad_pattern not in corrected else '❌'}")
    has_name_filter = 'first_name' in corrected.lower()
    has_john = "'john'" in corrected.lower()
    print(f"  4. Filters by student first_name: {'✅' if has_name_filter and has_john else '❌'}")
    
    print("\n✨ Expected: SELECT ... FROM students s JOIN enrollments e ... JOIN courses c ... WHERE s.first_name = 'john'")

if __name__ == "__main__":
    test_courses_by_john()
