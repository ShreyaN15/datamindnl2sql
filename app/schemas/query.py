...
class NL2SQLRequest(BaseModel):
    ...
    return_details: bool = Field(
        False,
        description=(
            "Return detailed validation information (single decode + validator; uses the "
            "same pattern corrector as the default path when use_sanitizer is true, but "
            "does not use the query enhancer or multi-candidate scoring from generate_sql)"
        ),
    )
...
class NL2SQLDetailedResponse(BaseModel):
    ...
    corrected_sql: str = Field(
        ...,
        description="SQL after validator fixes and optional pattern corrector (details path)",
    )
